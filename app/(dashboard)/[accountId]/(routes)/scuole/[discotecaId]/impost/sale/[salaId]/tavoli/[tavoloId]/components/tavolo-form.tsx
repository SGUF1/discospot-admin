...
const formSchema = z.object({
  ...
  numeroMassimo: z.coerce.number().min(1),
  prezzoPerPosto: z.boolean().default(false),
});
...
const TavoloForm = ({
  ...
  const description = initialData
    ? "Modifica il tavolo che verrà visualizzato nelle sale della discoteca"
    : "Crea un tavolo che verrà visualizzato nelle sale della discoteca";
  const toastMessage = initialData
    ? "Il tavolo è stato modificato"
    : "Il tavolo è stato creato";
  ...
  const onSubmit = async (data: TavoloFormValues) => {
    try {
      ...
      if (!initialData) {
        await axios.post(
          `/api/${params.accountId}/discoteche/${params.discotecaId}/impost/sale/${params.salaId}/tavoli`,
          data
        );
      } else {
        await axios.patch(
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 md:gap-5">
                    ...
                    >
                      ...
                      <SelectContent>
                        {posizioni.map((posizione) => (
                          <SelectItem key={posizione.id} value={posizione.id}>
                            {posizione.nome}
                          </SelectItem>
                        ))}
                      </SelectContent>
            ...
            <div className="flex flex-col space-y-5">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>Numero massimo di posti:</FormLabel>
                    <FormControl>
                      <Input
                        disabled={loading}
                        type="number"
