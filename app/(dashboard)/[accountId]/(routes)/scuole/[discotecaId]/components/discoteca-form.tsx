...
const formSchema = z.object({
  ...
  cap: z
    .string()
    .min(5, { message: "Il cap ha solo 5 numeri" })
    .max(5, { message: "Il cap ha solo 5 numeri" }),
...
const DiscotecaForm = ({ initialData, province }: DiscotecaFormProps) => {
  ...
  const title = initialData ? "Modifica scuola" : "Crea scuola";
  const description = initialData ? "Modifica a scuola" : "Crea a scuola";
  const toastMessage = initialData ? "Scuola aggiornata" : "Scuola creata";
  const action = initialData ? "Salva" : "Crea";
  ...
  const form = useForm<DiscotecaFormValues>({
    ...
    defaultValues: initialData
      ? {
          ...initialData,
          priority: parseInt(String(initialData?.priority)),
        }
      : {
          name: "",
          indirizzo: "",
  ...
  const onSubmit = async (data: DiscotecaFormValues) => {
    try {
      ...
      router.refresh();
      router.replace(`/${params.accountId}/scuole`);
  ...
  const onDelete = async () => {
    try {
      ...
      router.refresh();
      router.replace(`/${params.accountId}/scuole`);
      toast.success("La scuola è stata eliminata");
    } catch (error) {
        ...
        >
          <div className="grid grid-cols-5 space-x-5">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>Scuola image</FormLabel>
                  <FormControl>
                    <ImageUpload
            ...
            <div className="grid grid-cols-1 grid-row-2 justify-center items-center w-[102%]">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center self-start ">
                    <div className="space-x-0.5 w-2/3">
                      <FormLabel>È una scuola?</FormLabel>
                      <FormDescription>
                        Permette di essere alla scuola visibile nella sezione
                        scuole
                      </FormDescription>
                    </div>
                    <FormControl>
