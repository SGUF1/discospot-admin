...
const TavoloForm = ({ initialData, piani, posizioni, stati,  }: TavoloFormProps) => {
    ...
    const onSubmit = async (data: TavoloFormValues) => {
        try {
            setLoading(true);
            if (!initialData) {
                await axios.post(`/api/${params.accountId}/discoteche/${params.discotecaId}/impost/tavoli`, data);
            } else {
                await axios.patch(`/api/${params.accountId}/discoteche/${params.discotecaId}/impost/tavoli/${params.tavoloId}`, data)
            }
            router.refresh();
    ...
    const onDelete = async () => {
        try {
            setLoading(true);
            await axios.delete(`/api/${params.accountId}/discoteche/${params.discotecaId}/impost/tavoli/${params.tavoloId}`)
            router.refresh();
            router.replace(`/${params.accountId}/discoteche/${params.discotecaId}/impost`)
                    ...
                    <div className='grid grid-cols-4 space-x-5'>
                        ...
                        <div className='flex flex-col space-y-5'>
                            ...
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Piano tavolo:</FormLabel>
                                        <Select disabled={loading} onValueChange={field.onChange} value={field.value} defaultValue={field.value}>
                                            <FormControl>
                                                <SelectTrigger>
                                                    <SelectValue defaultValue={field.value} placeholder="Seleziona la provincia" />
