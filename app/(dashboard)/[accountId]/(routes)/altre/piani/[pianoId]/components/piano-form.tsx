...
type PianoFormValues = z.infer<typeof formSchema>
const PianoForm = ({ initialData }: PianoFormProps) => {
    ...
    const onSubmit = async (data: PianoFormValues) => {
        try {
            setLoading(true);
            if (!initialData) {
                await axios.post(`/api/${params.accountId}/altre/piani`, data);
            } else {
