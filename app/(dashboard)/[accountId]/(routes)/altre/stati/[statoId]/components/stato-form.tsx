...
import { Posizione, Stato, TipoInformazione } from '@prisma/client'
...
interface StatoFormProps {
    initialData: Stato | null,
}
...
const StatoForm = ({ initialData }: StatoFormProps) => {
    ...
    const onSubmit = async (data: StatoFormValues) => {
        try {
            setLoading(true);
            if (!initialData) {
                await axios.post(`/api/${params.accountId}/altre/stati`, data);
            } else {
                ...
                <form className='space-y-8 w-full ' onSubmit={form.handleSubmit(onSubmit)}>
                    <div className='grid grid-cols-4 space-x-5'>
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel>Nome dello stato:</FormLabel>
                                    <FormControl>
                                        <Input
                                            disabled={loading}
                                            placeholder="stato..."
